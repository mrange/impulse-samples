...
"#version 430\n"
 ...
 "{"
   "v.x=abs(v.x);"
   "float x=.5*max(c.x-c.y,0);"
   "return(v.x<x?"
     "abs(v.y):"
     "length(v-vec2(x,0)))-c.y;"
 "}"
 ...
 "{"
   "vec3 c=vec3(0);"
   "vec2 f=m[0].yz,y=(-f+2*gl_FragCoord.xy)/f.yy,r=(-f+2*m[1].xy)/f.yy,a=y,H=y;"
   "float l=m[0].x,n=sqrt(2)/f.y,z=n/x,B=n/.04,d=n/.25;"
   "r.y=-r.y;"
   "a/=x;"
   "a-=.5;"
   "H.x-=-.0375;"
   "H.y-=-.95;"
   "vec2 w=t(H),C=round(a);"
   "a-=C;"
   "C+=12*.5;"
   "H/=.04;"
   "int o=int(C.x+C.y*12+2);"
   "vec4 u=m[o];"
   "float g=u.x,F=u.z,E=s(y,vec2(.9)),e=s(a,vec2(.4))-.05;"
   "if(E<0.)"
     "{"
       "c+=s(vec3(.55,.5,.002))*(1./max(dot(a,a)+smoothstep(F+.125,F+3.,l),.001));"
       "vec2 A=a/.25;"
       "if(g<0)"
         "{"
   ...
   "if(w.y==0&&abs(w.x-.5)<5)"
     ...
     "}"
   "c+=s(vec3(.4,.5,.002))/max(length(y-r),.001);"
   "c=sqrt(c);"
   "v=vec4(c,1);"
 "}",

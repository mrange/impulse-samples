...
"#version 430\n"
 ...
 "{"
   ...
   "d.y=-d.y;"
   "vec3 r=vec3(0),l=vec3(s,0),C=vec3(d,1),z=normalize(l-vec3(0,0,10)),F=normalize(C-l),w=normalize(vec3(2,3,3));"
   "float o=v[0].x,H=v[0].w,G=sqrt(2)/y.y,E=G/a,D=G/.04,B=G/(.25*a);"
   "m/=a;"
   "m-=.5;"
   "i.x-=-.075/2;"
   "i.y-=-.95;"
   ...
   "vec4 I=v[int(A)];"
   "float g=I.x,J=I.z,K=t(m);"
   "vec3 L=x(m),M=sqrt(p(o));"
   "float N=1+dot(L,z);"
   "if(max(abs(n).x,abs(n).y)<.9)"
     "{"
       "vec3 O=r/4;"
       "vec2 P=vec2[6](vec2(0),vec2(.5,0),vec2(2),vec2(10,0),vec2(10,2),vec2(4,1))[int(g)];"
       "float Q=length(m);"
       "for(float R=0;R<P.y;++R)"
         "Q=min(abs(Q-.1),Q);"
       "vec3 R=(.2+p(2-g))*(P.x*.005/max(Q,.003));"
       "O=mix(O,R,smoothstep(E,-E,K));"
       "if(g<0)"
         "{"
           "vec2 S=m/.25;"
           "S.x+=-S.y/8;"
         ...
         "}"
       "vec3 T=p(3.5-s.y);"
       "O+=(pow(max(dot(w,reflect(z,L)),0.),20)/4+pow(max(dot(F,reflect(z,L)),0.),40)*M)*N*16*step(1,g);"
       "r=mix(r,O,smoothstep(E,-E,K));"
       "K=abs(K)-1/80.;"
       "r=mix(r,mix(vec3(1),T/3,smoothstep(J+1/8.,J+.5,o)),smoothstep(E,-E,K));"
     "}"
   "if(u.y==0&&abs(u.x-.5)<5)"
     "{"
       "vec3 T=p(-4.*s.y+(u.x<1?"
         "0:"
         "3));"
       "r=e(r,i,T,T*.075,D,mod(H*pow(10,u.x),10));"
     "}"

...
"#version 430\n"
 ...
 "{"
   ...
   "n.y-=.5;"
   "n=abs(n);"
   "float z=p(r,x),C=p(s,x),F=dot(normalize(vec2(1,-1)),n);"
 ...
 "{"
   ...
   "float C=s[0].x,F=s[1].z,z=s[1].w,d=sqrt(2)/x.y,w=d/m,I=d/.04,H=d/(.25*m),e=sign(l.y);"
   "f.y=-f.y;"
   ...
   "if(max(abs(i).x,abs(i).y)<.9)"
     "{"
       "vec4 K=s[int(u)];"
       "float L=K.x,M=K.z,N=h(a),O=smoothstep(M+1./2,M+1./8,C);"
       ...
       "if(L<1)"
         ...
         "}"
       "else"
         " Q=mix(Q,T,smoothstep(w,-w,N));"
       "vec3 U=t(3.5-y.y);"
       "Q+=(pow(max(dot(G,reflect(A,P)),0.),20)/4+pow(max(dot(E,reflect(A,P)),0.),40)*g)*(1+dot(P,A))*16*step(1,L);"
       "c=mix(c,Q,smoothstep(w,-w,N));"
       "N=abs(N)-1./80;"
       "c=mix(c,mix(U/3,vec3(1),O),smoothstep(w,-w,N));"
     "}"
